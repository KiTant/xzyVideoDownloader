...
class TitleMenu(CTkTitleMenu):
    def __init__(self, MainWindow: "MainWindowClass"):
        ...
        self.Settings_Submenu.add_option(option="Save settings",
                                         command=lambda: save_settings(MainWindow, FILES.get("settings_file")))

        self.Settings_Submenu.add_option(option="Load settings",
                                         command=lambda: load_settings(MainWindow, FILES.get("settings_file"), set_vars=True))
        ...
        self.Queue_Submenu.add_option(option="Save queue",
                                      command=lambda: save_settings(MainWindow, FILES.get("queue_file"), queue_obj=MainWindow.download_queue))
        self.Queue_Submenu.add_option(option="Load queue",
                                      command=lambda: load_settings(MainWindow, FILES.get("queue_file"), default_values={}, attr_name="download_queue"))

        self.Settings_Dropdown.add_option(option="Check updates", command=lambda: check_updates(MainWindow), accelerator="CmdOrCtrl+U")
